...
export default function ApiNavigation() {
  ...
  return (
    <>
      ...
      />
      <nav
        className={`h-screen bg-white py-4 max-lg:hidden fixed border-r-slate-[600/20] border-r overflow-y-auto ${
          expanded
            ? "min-w-[300px] overflow-y-auto"
            : "max-w-[240px] lg:w-[240px]"
        } duration-300 transition-all shadow-lg float-left`}
      >
...
function NavParent(props) {
  const [expanded, setExpanded] = useState(
    props.items && !props.collapsable ? true : false
  );
  ...
  return (
    <>
      <div
        className={`${
          expanded && props.collapsable ? "bg-gray-100 py-1 pb-2" : ""
        } max-lg:px-4 lg:px-3 ease-in-out mb-1 duration-300 w-full`}
      >
        {props.collapsable === false && props.href ? (
          <Link
            href={props.href.includes("#") ? props.href : `#${props.href}`}
            onClick={() => props.setIsOpenMobile(false)}
            className={`font-semibold text-[13px] max-lg:text-[1.6rem] flex items-center select-none my-1.5 ${
              !props.collapsable && "cursor-pointer hover:opacity-50"
            }`}
          >
            {props.title}
          </Link>
          ...
          >
            ...
            >
              {!props.collapsable ? null : expanded ? (
                <ArrowUpCircle
                  className="mx-2 max-lg:scale-[1.5] max-lg:mx-4"
                  size={17}
                />
              ) : (
                <ArrowDownCircle
                      ...
                      >
                        ...
                        {item.method &&
                          item.method.toUpperCase() === "POST" && (
                            <code className="text-[0.8em] text-green-600 border border-green-600 px-1 rounded ml-2">
                              POST
                            </code>
                          )}
                        {item.method && item.method.toUpperCase() === "GET" && (
                          <code className="text-[0.8em] text-red-600 border border-red-600 px-1 rounded ml-2">
                            GET
                          </code>
                        )}
                      </Link>
...
function MobileNavigation({ isOpenMobile, setIsOpenMobile }) {
  return (
    <>
      <div className="hidden max-lg:inline-block max-lg:fixed max-lg:w-full bg-white h-[65px] border-b-slate-300 border z-[100] max-lg:py-2">
        <div className="top-1/2 left-2 transform -translate-y-1/2 absolute">
          <h1 className="p-2 font-bold">Public API</h1>
        </div>
        <div
          onClick={() => setIsOpenMobile(!isOpenMobile)}
          id="hamburger-menu"
          className="absolute transform -translate-y-1/2 top-1/2 right-5 z-[10000]"
        >
