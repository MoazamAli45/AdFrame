...
export default function ResponseLine(props) {
  const supportLinkText = (
    <>
      Please file a issue report{" "}
      <a
        className="link"
        href={`mailto:${`info@example.com?subject=API documentation issue&body=We found an issue for endpoint: __ENDPOINT__`}`}
      >
        HERE
      </a>
      .
  ...
  const statusStyling = {
    ...
    black: {
      ...
      defaultDescription: (
        <>An undocumented error happened. {supportLinkText}</>
      ),
    },
    yellow: {
      ...
      defaultDescription: (
        <>
          Something happened, check the return debug object for more
          information.{" "}
        </>
      ),
    },
  ...
  return (
    <>
      <div className={`flex items-start flex-nowrap ${props.className || ""}`}>
        <Badge
          variant={"outline"}
          className={`font-normal px-1 rounded max-lg:mr-3 ${data.badgeClasses}`}
        >
        ...
        <p className="font-secondary text-[0.95rem]">
          {props.description || data.defaultDescription}{" "}
          {props.hasSupportLink && supportLinkText}
        </p>
      </div>
    </>
  );
